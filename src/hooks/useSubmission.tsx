...
import { validateConsistencyRatio, validateSubmission } from '@/lib/utils';
import { ROUTE_NAMES } from '@/router';
import { setErrors, setFormSubmitted, setRetry } from '@/store/pageStore';
import { setConsistencyRatio } from '@/store/surveyStore';
...
export default function useSubmission() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const cr_result = await validateConsistencyRatio({
        respondant: participant,
        survey: criteriaRanking,
      });
      if (typeof cr_result.message === 'number') {
        dispatch(setConsistencyRatio(cr_result.message));
      }

      if (cr_result.message > 0.1) {
        throw 'Ranking not consistency. Re-check priotisation.';
      }
